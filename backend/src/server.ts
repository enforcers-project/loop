...
app.get('/api/events', (req, res) => {
  ...
  if (sort === 'popular') {
    list = [...list].sort(
      (a, b) => b.rsvpCount + 2 * b.saveCount - (a.rsvpCount + 2 * a.saveCount),
    );
  } else if (sort === 'date') {
...
app.get('/api/events/:id/related', (req, res) => {
  ...
  const related = EVENTS.filter(
    (e) => e.id !== ev.id && e.category === ev.category,
  ).map(withOrganizer);
  ok(res, related.length ? related : EVENTS.filter((e) => e.id !== ev.id).slice(0, 3).map(withOrganizer));
});
