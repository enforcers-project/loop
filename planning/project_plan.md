...
### Attendee
*Permissions: discovers, saves, and RSVPs to events, follows organizers, comments, sets interests — and can host pickup runs.*
...
**Host capability** — any Attendee can post and run a pickup game:

- **Marcus, 28** — runs a Sunday soccer game and constantly chases people to confirm so he hits 10 players. He wants a roster that fills itself and shows spots remaining.
- **Leo, 19** — organizes weekend basketball runs and wants players to find the game, claim a spot, and see skill level so games stay balanced.
...
> **Note:** We consolidated the "Sports Host" concept into an Attendee/host capability rather than a third role, because hosting a run reuses the same create/view/manage verbs as ordinary attendee actions and needs no elevated permissions. Pickup runs are an event *type*, not a persona. (See §10, Decisions Log.)
...
**Access:** gated to `role='organizer'` **or** `is_host` (host path only when the Sports toggle is on / `is_sports`), matching `POST /api/events` auth and the `BottomBar` Create-tab gate (§8).
...
- **`BottomBar`** — mobile-only fixed tab bar on all primary screens; the elevated pink **Create** tab is gated to `role='organizer'` / `is_host`.
...
| `is_host` | `boolean` | Host **capability** — any attendee may post/manage pickup runs. Not a role. Default `false`. |
...
- **Four roles in the Figma export vs. the two-role constraint.** `project_knowledge.md` lists `attendee`/`organizer`/`promoter`/`sportsHost`. Resolved to a two-value `user_role` (`attendee`, `organizer`). `sportsHost` → the `users.is_host` boolean capability (any user can host pickup runs). `promoter` → the nullable `organizer_kind` display sub-type within the Organizer/Promoter role (drives only the pink RoleBadge tint, grants no extra permissions). Pickup runs are an event type (`events.is_sports` + `sports_details`), not a persona. This matches the Decisions Log in `user_stories.md`. The Figma `RoleBadge` "Sports Host" (green) variant, if kept, renders off `users.is_host` — it is not a role.
...
|---|---|---|---|
| **Collapse "Sports Host" into a host capability** — roles stay Attendee + Organizer/Promoter (`users.is_host` boolean; pickup runs are an event type via `events.is_sports` + `sports_details`). | The Figma export implied a 4-role model (attendee/organizer/promoter/sportsHost); hosting a run reuses the same create/view/manage verbs as ordinary attendee actions and needs no elevated permissions. | A 3rd (or 4th) standalone `sportsHost` role as the Figma export implied; a separate persona table. | Simpler permissions/role matrix and no dead role, vs. the RoleBadge "Sports Host" (green) tint now renders off a boolean (`is_host`), not a `user_role` value. |
